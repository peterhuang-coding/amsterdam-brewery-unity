(function(root,factory){
  ...
  'use strict';
  const GOALS=['market','club','parcel','noor','gate','garden','sorting','exit','barricade','market-stock','market-cold','market-shortcut','music'];
  const EVENTS=['route','market','club','parcel','noor','gate','garden','sorting','capacity','stuck','glimpse'];
  const NAMES={'market-stock':'退货货架','market-cold':'冷库','market-shortcut':'卸货卷门',music:'等待夜店下一段重拍',barricade:'后门摆放空桶的位置',market:'超市后场',club:'夜店后门',parcel:'冷藏箱',noor:'红灯街的 Noor',gate:'温室维护闸门',garden:'玻璃后的温室',sorting:'失物分拣场',exit:'回酒馆的出口'};
  ...
  function target(r){
    const market=B.locationInfo(r).market;
  ...
  function tick(r,dt){
    ...
    if(a.goal==='parcel'?B.nearest(r)?.id===to.id:dist(r.p,to)<(a.goal==='barricade'?7:18)){arrived(r);return;}
