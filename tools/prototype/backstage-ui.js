(function(root){
  ...
  class BackstageUI{
    ...
    render(){
      const r=this.run();if(!r)return;
      const auto=r.auto?.enabled,z=B.zone(r),near=B.nearest(r),street=B.streetInfo?.(r),tools=Boolean(r.street&&street&&street.id!=='legacy'),dragTarget=B.dragTarget?.(r),location=B.locationInfo?.(r);
      const signature=[r.sequence,z.id,near?.id,r.status,this.overview,auto,r.auto?.event,r.auto?.goal,r.auto?.speed,r.street?.situation,r.street?.dragging,location?.club?.phase,location?.door?.open,this.isPaused()].join('|');
